...
int main() {
  ...
  const auto config_path = sourceDir() / "configs" / "benchmarks" / "mesh_orientation_sweep.json";
  const auto output_root = sourceDir() / "outputs" / "test_mesh_orientation_sweep_smoke";
  const std::string command = "\"" + executable.string() + "\" --config \"" + config_path.string() +
                              "\" --output-dir \"" + output_root.string() + "\"";
  if (std::system(command.c_str()) != 0) {
    std::cerr << "Failed to run ex05_compare_backends for mesh_orientation_sweep.\n";
    return 1;
  }

  const auto summary_csv = output_root / "mesh_orientation_sweep" / "summary.csv";
  const std::string content = readTextFile(summary_csv);
  if (content.find("orientation_angle_deg") == std::string::npos || content.find("45.000000") == std::string::npos ||
      content.find("concave_l_block.obj") == std::string::npos) {
    std::cerr << "mesh_orientation_sweep summary.csv missing expected orientation labels.\n";
    return 1;
  }
  return 0;
}
