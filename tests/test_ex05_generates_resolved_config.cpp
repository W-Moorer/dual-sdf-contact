...
int main() {
  ...
  const auto output_root = sourceDir() / "outputs" / "test_ex05_generates_resolved_config";
  ...
  const auto resolved_config = output_root / "primitive_smoke" / "config_resolved.json";
  if (!std::filesystem::exists(resolved_config)) {
    std::cerr << "Missing config_resolved.json.\n";
    return 1;
  }

  const std::string content = readTextFile(resolved_config);
  if (content.find("\"config_summary\"") == std::string::npos ||
      content.find("\"resolved_sdf_backends\"") == std::string::npos) {
    std::cerr << "config_resolved.json missing expected fields.\n";
    return 1;
  }
  return 0;
}
